...
//	public String goBoard(HttpServletRequest request, HttpServletResponse response) {
	public Map<String, Object> goBoard(HttpServletRequest request, HttpServletResponse response) {
		...
		query = "SELECT "
				...
				+ " cq.Ins_Date_Time as Write_Time,"
				+ " cq.Page_Counter as page_Counter"
				+ " FROM CS_Ques cq "
		...
		if(searchKeyword != "" && condition != null) {
			...
			query += " GROUP BY cq.Board_ID, cq.Comment_ID,  cq.Title, cq.Ins_Date_Time, cq.page_Counter "
		...
		try {
			...
			while(rs.next()) {
				...
				qvo.setWrite_Time(rs.getString("Write_Time"));
				qvo.setPage_Counter(rs.getInt("page_Counter"));;
	...
	public void updateCurrentPageCounter(HttpServletRequest request, HttpServletResponse response) {
	    // 변수 선언 및 초기화
	    String page_Counter = request.getParameter("page_Counter");
	    String boardID = request.getParameter("boardID");
	    System.out.println(page_Counter);
	    System.out.println(boardID);

	    PreparedStatement ps = null; // PreparedStatement를 메서드 레벨 변수로 선언

	    try {
	        // UPDATE 쿼리 생성
	        String query = "UPDATE CS_Ques "
	                + " SET Page_Counter = ? "
	                + " WHERE Board_ID = ? ";

	        con = db.getConnection();
	        ps = con.prepareStatement(query);
	        ps.setString(1, page_Counter);
	        ...
	        // PreparedStatement 및 Connection 닫음
	        db.close(ps);
	        db.close(con);
	    }
	}
...
//	public String deleteBoard(HttpServletRequest request, HttpServletResponse response) {
	...
	public String boardInfo(HttpServletRequest request, HttpServletResponse response) {
		...
		query = "SELECT * FROM ("
				...
				+ " LEFT JOIN CS_ANS ca ON cq.Board_ID = ca.Board_ID "
				+ " WHERE cq.Del_Yn <> 'Y' OR cq.Del_Yn IS NULL "
				+ " GROUP BY cq.Board_ID, cq.Comment_ID, cq.contents, cq.File_Name, cq.Title, cq.page_Counter, cq.Ins_Date_Time "
				+ " ) AS boardInfoTable "
				+ " WHERE Board_ID = "+ request.getParameter("board_ID");
