...
function isNewPost(Write_Time) {
    var currentTime = new Date();
    var postTime = new Date(Write_Time);

    // 게시글 작성 시간과 현재 시간의 차이를 계산 (밀리초 단위)
    var timeDifference = currentTime - postTime;

    // 24시간 이내에 작성된 글이면 true 반환, 그렇지 않으면 false 반환
    if(timeDifference <= 24 * 60 * 60 * 1000 ){
    	var newImage = document.createElement("img");
    	newImage.src = "images/new.png";
    	newImage.width = 30; 
    	newImage.alt = "new";
    	return newImage;
    }else{
...
function Isread(page_counter) {
    var page_counter = parseInt(page_counter);
    
    if (page_counter > 0) {
        var checkedImage = document.createElement("img");
        checkedImage.src = "images/checked.jpg";
        checkedImage.width = 30;
        checkedImage.alt = "checked";
        return checkedImage;
    } else {
        return ""; // 이미지를 반환하지 않는 경우 빈 문자열 반환
    }
}
	   ...
	   /*  processData: false,
	    ...
	    success: function(data) {
	    	...
	    	if(Array.isArray(bList) && bList.length > 0){
				...
				comment_html += "<th scope='col'>조회수</th>";
				comment_html += "</tr>";
				comment_html += "</thead>";
				comment_html += "<tr>";
				comment_html += "<tbody>";
				...
				for(i = 0;i < bList.length;i++){
					...
					var write_time = bList[i].Write_Time;
					var isNew = isNewPost(ins_Date_Time);
					var write_time = bList[i].Write_Time;
					var page_counter = bList[i].page_Counter;
			        var newBadge = isNew ? "<span class='newBadge'>new</span>" : "";
					...
					comment_html += "<td>";
					var newImage = isNewPost(write_time);
					if (newImage) {
					    comment_html += newImage.outerHTML; // <img> 요소를 추가합니다.
					}
					comment_html += title;
					var readImage = Isread(page_counter);
					if (readImage) {
						comment_html += readImage.outerHTML;
					}
					comment_html += "</td>";
					
					
					//if(isNewPost(wirte_Time) == true){
					//	comment_html += "<td>new</td>";
					//}
					//else{
					//	comment_html += "<td>안뜸</td>";
					//}
					
					comment_html += "<td>" + comment_ID + "</td>";
					comment_html += "<td>" + ins_Date_Time + "</td>";
					comment_html += "<td>" + page_counter + "</td>";
					comment_html += "</tr>";
